<?php if ($sf_user->hasFlash('notice')): ?>
  <div class="notice flash_message">
    <?php if ($sf_user->hasFlash('notice_detail')): ?>
      <span class="expander"><?php echo link_to_function("[".__('show', array(), 'messages')."]", "jQuery('#notice_detail_msgs').show(); jQuery(this).hide(); jQuery(this).hide();") ?></span>
    <?php endif ?>
    <?php $params = $sf_user->getFlash('notice_params', array()) ?>
    <?php echo __($sf_user->getFlash('notice'), $params instanceOf sfOutputEscaper? $params->getRawValue() : $params, 'messages') ?>
  </div>
<?php endif; ?>
  ...
  <div class="notice_detail flash_message" style="display: none" id="notice_detail_msgs">
    <ul>
      ...
      <?php foreach ($sf_user->getFlash("notice_detail") as $value): ?>
        <li><?php echo __(strval($value), $params instanceOf sfOutputEscaper? $params->getRawValue() : $params, 'messages') ?></li>
      <?php endforeach ?>
    </ul>
  </div>
<?php endif ?>
...
<?php if ($sf_user->hasFlash('error')): ?>
  <div class="error flash_message">
    <?php if ($sf_user->hasFlash('error_detail')): ?>
      <span class="expander"><?php echo link_to_function("[".__('show', array(), 'messages')."]", "jQuery('#error_detail_msgs').show(); jQuery(this).hide(); jQuery(this).hide();") ?></span>
    <?php endif ?>
    <?php $params = $sf_user->getFlash('error_params', array()) ?>
    <?php echo __($sf_user->getFlash('error'), $params instanceOf sfOutputEscaper? $params->getRawValue() : $params, 'messages') ?>
  </div>
<?php endif; ?>
...
<?php if ($sf_user->hasFlash('warning')): ?>
  ...
  <div class="warning flash_message">
    <?php if ($sf_user->hasFlash('warning_detail')): ?>
      <span class="expander"><?php echo link_to_function("[".__('show', array(), 'messages')."]", "jQuery('#warning_detail_msgs').show(); jQuery(this).hide(); jQuery(this).hide();") ?></span>
    <?php endif ?>
    <?php $params = $sf_user->getFlash('warning_params', array()) ?>
    <?php echo __($sf_user->getFlash('warning'), $params instanceOf sfOutputEscaper? $params->getRawValue() : $params, 'messages') ?>
  </div>
